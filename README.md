# AI Team Agent System

AI 팀 에이전트 시스템의 초기 프로젝트 구조입니다. 첫 버전은 구현보다 역할 분리, 문서화 위치, 테스트/예시 자료를 담을 공간을 명확히 만드는 데 집중합니다.

## 언어 정책

- 내부 명령어, 파일 이름, 시스템 프롬프트는 영어로 작성합니다.
- 사용자에게 보이는 출력, 보고서, 요약, 알림, 설명은 한국어로 작성합니다.
- 외부 API는 아직 연결하지 않습니다.

## 현재 범위

- 에이전트별 디렉터리와 기본 시스템 프롬프트를 준비했습니다.
- 템플릿, 하네스 테스트 케이스, 골든 예시, 실패 케이스, 문서 디렉터리를 준비했습니다.
- Google Sheets, Gmail, Slack, Telegram, Instagram, reporting 연동은 TODO로만 남겨두었습니다.

## 프로젝트 구조

```text
.
├── AGENTS.md
├── CLAUDE.md
├── README.md
├── agents
│   ├── assistant
│   ├── communication
│   ├── document
│   ├── email
│   ├── harness
│   ├── instagram
│   ├── notification
│   ├── sheets
│   ├── sheets_write
│   ├── supervisor
│   └── workflow
├── docs
│   └── architecture.md
├── examples
│   ├── failure
│   └── golden
├── templates
└── tests
    └── harness
        ├── checklists
        └── replay
```

## 에이전트 역할

- `supervisor`: 작업 분배, 정책 적용, 결과 통합을 담당하며 로컬 MVP 라우터로 요청을 `document`, `communication`, `sheets_reader`, `assistant_report`, `harness`, `web_dashboard`, `blocked` 중 하나로 분류합니다.
- `assistant`: 일반 지원, 사용자 요청 정리, 간단한 산출물 초안을 담당합니다.
- `document`: 문서 생성, 요약, 구조화된 문서 산출을 담당합니다.
- `communication`: 이메일, 메신저, 알림 등 커뮤니케이션 흐름을 담당할 예정입니다.
- `email`: 로컬 샘플 이메일을 요약하고 한국어 답장 초안과 승인 요청서를 생성하며 실제 발송은 하지 않습니다.
- `notification`: Slack/Telegram 메시지 초안만 생성하며 실제 발송은 하지 않습니다.
- `sheets`: 로컬 CSV 샘플을 읽기 전용으로 분석하며 Google Sheets API에 연결하지 않습니다.
- `sheets_write`: Google Sheets 쓰기 제안을 승인 요청서로만 변환하며 실제 쓰기는 수행하지 않습니다.
- `workflow`: 반복 업무, 상태 전이, 작업 순서를 관리할 예정입니다.
- `harness`: 테스트 실행, 평가 기준, 회귀 검증을 담당할 예정입니다.
- `instagram`: Instagram 관련 콘텐츠 준비와 게시 흐름을 담당할 예정입니다.

## TODO

### Google Sheets

- 스프레드시트 읽기/쓰기 권한 범위를 정의합니다.
- 데이터 입력, 상태 추적, 리포트 적재용 시트 구조를 설계합니다.
- 외부 API 연결 전 테스트 더블 또는 정적 샘플 데이터를 준비합니다.
- Google Sheets 쓰기 작업은 자동 실행하면 안 됩니다.
- Google Sheets 쓰기 작업은 명시적인 사용자 승인 이후에만 실행할 수 있습니다.
- 쓰기 전에는 대상 스프레드시트, 대상 탭, 대상 행/열, 원본 값, 제안 값을 한국어로 보여줘야 합니다.
- 쓰기 후에는 기록된 값을 다시 확인하고, 검증 결과를 한국어로 보고해야 합니다.
- Sheets Reader Agent는 읽기 전용입니다.
- Sheets Write Approval Agent는 승인 요청 전용입니다.
- 실제 Google Sheets 쓰기 작업은 아직 연결하지 않았습니다.
- 향후 쓰기 작업은 실행 전에 대상 스프레드시트, 탭, 행, 열, 원본 값, 제안 값, 승인 필요 여부, 사후 검증 계획을 먼저 표시해야 합니다.

### Gmail

- 수신함 요약, 답장 초안, 후속 조치 추출 범위를 정의합니다.
- 사용자 승인 없이 발송하지 않는 정책을 명문화합니다.
- 샘플 메일 기반 하네스 테스트 케이스를 추가합니다.
- Email Draft Agent는 로컬 초안 전용입니다.
- Gmail API는 아직 연결하지 않았습니다.
- 실제 이메일 발송은 차단되어 있습니다.
- 향후 이메일 발송은 명시적인 사용자 승인 이후에만 검토할 수 있습니다.
- 수신자, 회사명, 마감일, 금액, 약속, 법적/권리 이슈가 불명확한 이메일은 `확인 필요`로 표시해야 합니다.

### Slack

- 채널 요약, DM 답장 초안, 알림 우선순위 규칙을 정의합니다.
- 실제 전송 전 사용자 확인 흐름을 설계합니다.
- Slack 메시지 샘플과 실패 케이스를 준비합니다.
- Notification Draft Agent는 Slack 상태 요약 초안만 생성하며 Slack API 연결과 실제 전송은 수행하지 않습니다.

### Telegram

- 알림, 명령 수신, 상태 보고 목적을 구분합니다.
- 봇 토큰과 채팅 ID는 환경 변수로 관리하는 정책을 문서화합니다.
- 연결 전 로컬 모의 이벤트 형식을 정의합니다.
- 알림 정책이 완전히 정의되기 전까지 Telegram 알림은 명시적인 사용자 승인이 필요합니다.
- MVP 단계에서는 어떤 외부 알림도 자동 전송하지 않습니다.
- Telegram은 사용자 승인 후에만 초안 알림, 승인 요청, 개인 보고 용도로 사용할 수 있습니다.
- Notification Draft Agent는 Telegram 긴급 알림 초안과 승인 요청 초안만 생성하며 Telegram API 연결과 실제 전송은 수행하지 않습니다.

### Instagram

- 콘텐츠 캘린더, 캡션 초안, 해시태그 추천 범위를 정의합니다.
- 게시 권한과 승인 절차를 분리합니다.
- 이미지/영상 자산 검증 규칙을 추가합니다.

### Reporting

- 일간/주간 보고서 형식을 정의합니다.
- 에이전트별 작업 결과를 하나의 한국어 요약으로 통합하는 규칙을 정합니다.
- 성공/실패/보류 상태의 공통 스키마를 설계합니다.
- Assistant Report는 Supervisor 라우팅 결과를 반영해 총 요청 수, 차단된 요청 수, 승인 필요 요청 수, 라우팅 대상, 위험 작업 차단 요약, 확인 필요 요청을 기록합니다.

## 다음에 확인할 것

- 디렉터리 이름과 에이전트 역할이 실제 운영 계획과 맞는지 확인합니다.
- 외부 서비스별 승인 정책과 보안 경계를 먼저 확정합니다.
- 하네스 테스트 케이스와 골든 예시의 최소 샘플을 추가합니다.

## 웹 대시보드

- `apps/web`은 Vercel에 배포된 정적 Next.js 대시보드 MVP입니다.
- 배포 상태: 배포 완료
- 현재 Vercel URL: https://ai-team-agent-one.vercel.app
- Dashboard type: static MVP
- Root Directory: `apps/web`
- Install Command: `npm install`
- Build Command: `npm run build`
- Environment variables: none
- External APIs: none
- 외부 API, 인증 정보, 환경 변수, 실서비스 데이터 연결은 추가하지 않았습니다.
- 대시보드는 Assistant Report Agent가 생성한 `reports/daily_status_report.md`의 주요 항목을 표시합니다.
- 대시보드는 Supervisor 라우팅 상태를 표시합니다.
- 대시보드는 Notification Draft Agent의 Slack/Telegram 초안 상태를 표시합니다.
- 대시보드는 Email Draft Workflow 상태를 표시합니다.
- Gmail API는 아직 연결하지 않았습니다.
- 실제 이메일 발송은 계속 차단되어 있습니다.
- 향후 이메일 발송은 명시적인 사용자 승인 이후에만 검토할 수 있습니다.
- 대시보드는 Sheets Write Approval Flow 상태를 표시합니다.
- 실제 Google Sheets 쓰기 작업은 아직 연결하지 않았습니다.
- Google Sheets API는 아직 연결하지 않았습니다.
- 향후 모든 Google Sheets 쓰기 작업은 계속 승인 게이트를 거쳐야 합니다.
- 향후 쓰기 작업은 대상 스프레드시트, 대상 탭, 대상 행, 대상 열, 원본 값, 제안 값, 사후 검증 계획을 실행 전에 표시해야 합니다.
- Slack/Telegram 실제 발송은 연결하지 않았습니다.
- 모든 외부 실행 작업은 계속 승인 게이트 또는 차단 상태로 유지합니다.
- 현재 보고서 연동은 정적/build-time 방식이며, 외부 API나 런타임 파일 동기화는 사용하지 않습니다.
- Assistant Report 데이터는 보고서 생성 시점의 로컬 상태를 반영할 수 있으며, 최신 Git 상태는 Vercel/GitHub 배포 기준으로 별도 확인해야 합니다.
- `postcss` 관련 npm audit 리스크는 `apps/web/package.json`의 npm override로 `postcss`를 `^8.5.10` 이상으로 고정해 해소했습니다.
- 현재 웹 앱의 `npm audit`는 `found 0 vulnerabilities` 상태입니다.
- `npm audit fix --force`는 breaking change를 만들거나 Next.js를 다운그레이드할 수 있어 사용하지 않았습니다.
- Vercel 설정은 Root Directory `apps/web`, Framework Preset `Next.js`, Build Command `npm run build`, Install Command `npm install`을 사용합니다.
- 향후 재배포는 `npm audit`, `npm run build`, `python3 tests/harness/run_all.py`가 다시 통과한 뒤에만 진행해야 합니다.

## 하네스 통합 점검

하네스 정책, 리플레이 데이터셋, 문서 검증, 문서 워크플로, 커뮤니케이션 검증, 커뮤니케이션 워크플로, Email Draft 검증, Sheets Reader 검증, Sheets Write Approval 검증, Assistant Report 검증, Supervisor 라우팅 검증, Notification 초안 검증, Python 문법 검사를 한 번에 실행하려면 다음 명령어를 사용합니다.

```bash
python3 tests/harness/run_all.py
```

개별 정책 체커만 실행하려면 다음 명령어를 사용합니다.

```bash
python3 tests/harness/policy_checker.py
```

통합 점검은 결과를 한국어로 출력합니다. 하위 점검 중 하나라도 실패하면 0이 아닌 종료 코드를 반환합니다.

Supervisor Agent MVP 라우터만 실행하려면 다음 명령어를 사용합니다.

```bash
python3 agents/supervisor/supervisor_router.py
python3 tests/harness/supervisor_validation_runner.py
```

Notification Draft Agent MVP 초안 생성과 검증만 실행하려면 다음 명령어를 사용합니다.

```bash
python3 agents/notification/notification_draft_runner.py
python3 tests/harness/notification_validation_runner.py
```

Email Draft Workflow MVP 요약, 답장 초안, 승인 요청서 생성과 검증만 실행하려면 다음 명령어를 사용합니다.

```bash
python3 agents/email/email_draft_runner.py
python3 tests/harness/email_draft_validation_runner.py
```

Sheets Write Approval Flow MVP 승인 요청서 생성과 검증만 실행하려면 다음 명령어를 사용합니다.

```bash
python3 agents/sheets_write/sheets_write_approval_runner.py
python3 tests/harness/sheets_write_approval_validation_runner.py
```
